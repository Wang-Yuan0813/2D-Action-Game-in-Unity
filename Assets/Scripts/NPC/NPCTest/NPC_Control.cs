// Codex edit verification comment.
...
public class NPC_Control : MonoBehaviour
{
    ...
    [Header("NPC 气泡显示距离")]
    [Min(0f)] public float bubbleDisplayDistance = 8f;
    [Header("NPC 常态深色系数")]
    ...
    void Start()
    {
        ...
        bubbleControl = bubble.GetComponent<SpeakBubble_Control>();
    ...
    private IEnumerator BubbleTriggerActionCoroutine()//这里把该NPC的气泡内容加进去
    {
        while (true)
        {
            ...
            if (playerIsWithinBubbleDistance && bubbleControl.canBubbleActive)
            {
                string bubbleContent = npcSpeaking.bubbleList[Random.Range(0, npcSpeaking.bubbleList.Count)];
                bubbleControl.SetText(bubbleContent);
                Debug.Log("[" + this.name + "]" + "bubbleContent = " + bubbleContent);
            }
        }
    }
    private void UpdateBubbleDistanceState()
    {
        ...
        bool wasWithinDistance = playerIsWithinBubbleDistance;
        playerIsWithinBubbleDistance = playerTransform != null &&
            Vector2.Distance(transform.position, playerTransform.position) <= bubbleDisplayDistance;

        if (wasWithinDistance && !playerIsWithinBubbleDistance)
        {
            bubbleControl.BubbleDisappear(0f);
        }
    }
